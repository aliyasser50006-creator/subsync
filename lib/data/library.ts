...
export function parseLibraryCategory(value: string | string[] | undefined) {
  const cat = firstValue(value);
  return cat === 'all' || !cat ? undefined : cat;
}
...
export async function getLibraryPageData({
  ...
  if (categoryId) {
    const { data: jobCats } = await supabase.from('job_categories').select('job_id').eq('category_id', categoryId);
    const catJobIds = (jobCats || []).map(jc => jc.job_id);
    if (catJobIds.length === 0) {
      return {
        ...
        sort,
        categoryId,
        stats: { totalCount: 0, readyCount: 0, processingCount: 0 },
      };
    }
    pageQuery = pageQuery.in('id', catJobIds);
  }
