...
export async function getJobs(searchQuery: string, isAvailable: boolean): Promise<{ data?: Job[]; error?: string }> {
  try {
    const supabase = await createClient();
    const { data: { user }, error: userError } = await supabase.auth.getUser();
    if (userError || !user) return { error: 'Unauthorized' };
    ...
    if (isAvailable) {
      query = query.in('status', ['ready', 'done']);
    }
...
export async function updateJob(id: string, title: string, videoUrl: string, imgUrl: string): Promise<{ error?: string }> {
  try {
    const supabase = await createClient();
    const { data: { user }, error: userError } = await supabase.auth.getUser();
    if (userError || !user) return { error: 'Unauthorized' };
