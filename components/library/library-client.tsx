...
export function LibraryClient({
  ...
  useEffect(() => setSelectedCategory(categoryId || 'all'), [categoryId]);
  ...
  const handleStatusChange = (value: string) => {
    ...
    navigate({ page: 1, status, sort: selectedSort, categoryId: selectedCategory });
  };

  const handleSortChange = (value: string) => {
    const sort = value as LibrarySort;
    setSelectedSort(sort);
    navigate({ page: 1, status: selectedStatus, sort, categoryId: selectedCategory });
  };

  const handleCategoryChange = (value: string) => {
    setSelectedCategory(value);
    navigate({ page: 1, status: selectedStatus, sort: selectedSort, categoryId: value });
  };
  ...
  const handleClearFilters = () => {
    ...
    navigate({ path: '/library', page: 1, query: '', status: 'all', sort: selectedSort, categoryId: 'all' });
  };
  ...
  return (
    <>
        <LibraryToolbar
          ...
          isPending={isPending || isRandomGenerating}
